 ...
 */
class CalendarWeekViewRenderer extends AbstractRenderer {
	...
	protected void renderTagContent(Map attrs, Closure body, MarkupBuilder builder) throws RenderException {
		...
		int startHour = 6
		if (attrs.startHour) {
			startHour = attrs.startHour as Integer
		}
		...
		int endHour = 22
		if (attrs.endHour) {
			endHour = attrs.endHour as Integer
		}
		...
		builder.table("class": "calendar") {
			thead {
				tr {
					...
					Calendar calendar = new GregorianCalendar(time: date)
					...
					tr("class": "${hour % 2 == 0 ? 'evenHour' : 'oddHour'}") {
										...
										if (!addedEvents.containsKey(eventItem.id)) {
											...
											if (attrs.teaser && attrs.teaser == "true") {
												...
												if (attrs.teaserLength) {
													try {
														teaserLength = attrs.teaserLength as Integer
													}
													catch (e) {
						...
						for (int i = 1; i <= 3; i++) {
							tr("class": "${hour % 2 == 0 ? 'evenHour' : 'oddHour'}") {
								...
								Calendar cal = new GregorianCalendar(time: date)
	...
	protected List<Resource> getComponentResources(Map attrs, String resourcePath) throws RenderException {
		...
		if (!attrs.skin || attrs.skin == "default") {
			css.builder.link(rel: "stylesheet", type: "text/css", href: "${resourcePath}/css/calendarweekview.css")
	...
	protected void renderResourcesContent(Map attrs, MarkupBuilder builder, String resourcePath) throws RenderException {
		builder.yield("<!-- Calendar Month View -->", false)
